/*
 * Apple macOS AVAudioPlayer routine added by Genki Ishida and Kenta Kubo
 *
...
int dr_load_sample(char const* const filename)
{
	...
	AVAudioPlayer* const m = [[AVAudioPlayer alloc] initWithContentsOfURL: [NSURL fileURLWithPath: s] error: nil];
	if (!m) {
...
{
	AVAudioPlayer* const m = [movies_WAV objectAtIndex: key];
